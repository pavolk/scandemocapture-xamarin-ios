...
namespace SocketMobile
{
    namespace Capture
    {
        ...
        public partial class CaptureHelper
        {
            ...
            private List<CaptureHelperDevice> _managerDevices = new List<CaptureHelperDevice>();
            ...
            /// <summary>
            /// return a shallow copy of the connected dmanager evices list. Since this list is
            /// initially used by CaptureHelper, adding or removing manager device will have no
            /// effect.
            /// </summary>
            /// <returns>a list of CaptureHelperDevice</returns>
            public List<CaptureHelperDevice> GetManagerDevicesList()
            {
                return new List<CaptureHelperDevice>(_managerDevices);
            }
            ...
            public Task<DataConfirmationActionResult> GetDataConfirmationActionAsync()
            {
                ...
                Task<DataConfirmationActionResult> result = propertyResult.ContinueWith(res =>
                {
                    int beep = 0;
                    int led = 0;
                    int rumble = 0;
            ...
            private EventHandler<DeviceArgs> deviceManagerArrival;
            /// <summary>
            /// DeviceManagerArrival event fires when a device
            /// manager connects to the host. The argument
            /// contains the CaptureHelperDevice that should
            /// be used in order to control this particular device manager.
            /// At this point CaptureHelper has already open the device manager
            /// so the application can receive notifications from this device manager.
            /// </summary>
            public event EventHandler<DeviceArgs> DeviceManagerArrival
            {
                add
                {
                    deviceManagerArrival += value;
                    lock (_managerDevices)
                    {
                        if (_managerDevices.Count > 0)
                        {
                            foreach (CaptureHelperDevice device in _managerDevices)
                            {
                                OnDeviceManagerArrival(device);
                            }
                        }
                    }
                }
            ...
            /// <summary>
            /// DeviceManagerRemoval event fires when a device manager disconnects
            /// from the host. The CaptureHelperDevice passed in argument
            /// is for reference only. Whatever operation is done with
            /// this CaptureHelperDevice will fail.
            /// </summary>
            public event EventHandler<DeviceArgs> DeviceManagerRemoval;
            ...
            internal async Task<long> DoFireCaptureEventAsync(ICapture.CaptureEventArgs e)
            {
                ...
                switch (e.captureEvent.EventID)
                {
                    ...
                    case ICaptureEvent.Id.kDeviceManagerArrival:
                        {
                            ICaptureDevice device = SktClassFactory.createDeviceInstance(_capture);
#if DEBUG
                            string manager = "";
                            if(e.captureEvent.EventID == ICaptureEvent.Id.kDeviceManagerArrival)
                            {
                              manager = "Manager ";
                            }
                            if (DebugConsole != null)
                            {
                                DebugConsole.PrintLine("about to open the Capture Device " + manager + e.captureEvent.DeviceInfo.Guid);
                            }
#endif
                            ...
                            if (DebugConsole != null)
                            {
                                DebugConsole.PrintLine("done opening the Capture Device " + manager + e.captureEvent.DeviceInfo.Guid + " : " + result);
                            }
#endif
                            if (SktErrors.SKTSUCCESS(result))
                            {
                                List<CaptureHelperDevice> devices = _devices;
                                if (e.captureEvent.EventID == ICaptureEvent.Id.kDeviceManagerArrival)
                                {
                                    devices = _managerDevices;
                                }
                                // attach the device info to the capture device interface
                                CaptureHelperDevice helperDevice = null;
                                lock (devices)
                                {
                                    device.DeviceInfo = e.captureEvent.DeviceInfo;
                                    helperDevice = new CaptureHelperDevice(this, device, true) { HasOwnership = true };
                                    devices.Add(helperDevice);
                                }
                                if (e.captureEvent.EventID == ICaptureEvent.Id.kDeviceManagerArrival)
                                {
                                    OnDeviceManagerArrival(helperDevice);
                                }
                                else
                                {
                    ...
                    case ICaptureEvent.Id.kDeviceManagerRemoval:
                        {
                            CaptureHelperDevice deviceFound = null;
                            List<CaptureHelperDevice> devices = _devices;
                            if (e.captureEvent.EventID == ICaptureEvent.Id.kDeviceManagerRemoval)
                            {
                                devices = _managerDevices;
                            }
                            ...
                            lock (devices)
                            {
                                deviceFound = devices.Find(device => device.GetDeviceInfo().Guid == e.captureEvent.DeviceInfo.Guid);
                            ...
                            if (deviceFound != null)
                            {
                                ...
                                if (SktErrors.SKTSUCCESS(result))
                                {
                                    if (deviceFound != null)
                                    {
                                        if (e.captureEvent.EventID == ICaptureEvent.Id.kDeviceManagerRemoval)
                                        {
                                            OnDeviceManagerRemoval(deviceFound);
                                        }
                                        else
                                        {
            ...
            internal void OnDeviceManagerArrival(CaptureHelperDevice device)
            {
                EventHandler<DeviceArgs> handler = deviceManagerArrival;
            ...
            internal void OnDeviceManagerRemoval(CaptureHelperDevice device)
            {
                EventHandler<DeviceArgs> handler = DeviceManagerRemoval;
        ...
        public partial class CaptureHelperDevice
        {
            ...
            /// <summary>
            /// contains the data source information result
            /// </summary>
            public class DataSourceResult : CaptureHelper.AsyncResult
            {
                /// <summary>
                /// contains the actual state of the data source
                /// </summary>
                public enum eStatus
                {
                    /// <summary>
                    ...
                    disable,
                    /// <summary>
                    /// the data source is not supported on this device
                    /// </summary>
                    notSupported
                };
                /// <summary>
                /// defines the first symbology data source ID. This can be useful when requesting the
                /// state of all the symbologies of a device starting with ID = FirstDataSourceSymbologyID
                /// to ID = LastDataSourceSymbologyID.
                /// NOTE: Symbology ID 0 means unknown Symbology.
                /// </summary>
                public const int FirstDataSourceSymbologyID = ICaptureDataSource.Id.kSymbologyAustraliaPost;
                /// <summary>
                /// defines the last symbology data source ID. This can be useful when requesting the
                /// state of all the symbologies of a device starting with ID = FirstDataSourceSymbologyID
                /// to ID = LastDataSourceSymbologyID.
                /// NOTE: Symbology ID 0 means unknown Symbology.
                /// </summary>
                public const int LastDataSourceSymbologyID = ICaptureDataSource.Id.kLastSymbologyID;
                /// <summary>
                /// contains the actual Data Source ID.
                /// </summary>
                public int ID;
                /// <summary>
                /// contains the Data Source status.
                /// </summary>
                public eStatus Status;
                /// <summary>
                /// contains the Data Source name.
                /// </summary>
                public string Name;
            }
            /// <summary>
            /// retrieve the information about a data source identified by its ID.
            /// the data source IDs go from DataSourceResult.FirstDataSourceSymbologyID to
            /// DataSourceResult.LastDataSourceSymbologyID (excluded).
            /// The ID of a specific data source can be discovered by typing ICaptureDataSource.Id.
            /// </summary>
            /// <param name="DataSourceId"></param>
            /// <returns>
            /// the data source information and a result code set to SktErrors.ESKT_NOERROR in
            /// case of success or to an error code otherwise.
            /// </returns>
            public Task<DataSourceResult> GetDataSourceAsync(int DataSourceId)
            {
                ICaptureProperty property = SktClassFactory.createCaptureProperty();
                property.ID = ICaptureProperty.PropId.kDataSourceDevice;
                property.Type = ICaptureProperty.Types.kDataSource;
                property.DataSource.ID = DataSourceId;
                property.DataSource.Flags = ICaptureDataSource.FlagsMask.kStatus;
                Task<PropertyResult> resultGetProperty = CaptureDevice.GetPropertyAsync(property);
                Task<DataSourceResult> result = resultGetProperty.ContinueWith(res =>
                {
                    DataSourceResult symbologyResult;

                    if (res.Result.IsSuccessful())
                    {
                        DataSourceResult.eStatus symbologyStatus;

                        switch (res.Result.Property.DataSource.Status)
                        {
                            case ICaptureDataSource.StatusValues.kEnable:
                                symbologyStatus = DataSourceResult.eStatus.enable;
                                break;
                            case ICaptureDataSource.StatusValues.kDisable:
                                symbologyStatus = DataSourceResult.eStatus.disable;
                                break;
                            case ICaptureDataSource.StatusValues.kNotSupported:
                            default:
                                symbologyStatus = DataSourceResult.eStatus.notSupported;
                                break;
                        }
                        symbologyResult = new DataSourceResult()
                        {
                            Result = res.Result.Result,
                            ID = res.Result.Property.DataSource.ID,
                            Status = symbologyStatus,
                            Name = res.Result.Property.DataSource.Name
                        };
                    }
                    else
                    {
                        symbologyResult = new DataSourceResult() { Result = res.Result.Result };
                    }
            ...
            /// The data source ID can be find out by typing: ICaptureDataSource.Id.
            /// </summary>
            /// <param name="DataSourceId">the data source to enable or disable</param>
            /// <param name="enable">true to enable the data source, false otherwise</param>
            /// <returns>
            /// a result code set to SktErrors.ESKT_NOERROR in case of success or
            /// to an error code otherwise
            /// </returns>
            public Task<CaptureHelper.AsyncResult> SetDataSourceAsync(int DataSourceId, bool enable)
            {
                ICaptureProperty property = SktClassFactory.createCaptureProperty();
                property.ID = ICaptureProperty.PropId.kDataSourceDevice;
                property.Type = ICaptureProperty.Types.kDataSource;
                ...
                if (enable)
                {
                    property.DataSource.Status = ICaptureDataSource.StatusValues.kEnable;
                }
                else
                {
                    property.DataSource.Status = ICaptureDataSource.StatusValues.kDisable;
                }
                Task<PropertyResult> resultSetProperty = CaptureDevice.SetPropertyAsync(property);
                Task<CaptureHelper.AsyncResult> result = resultSetProperty.ContinueWith(res =>
                {
                    CaptureHelper.AsyncResult resultDataSource = new CaptureHelper.AsyncResult() { Result = res.Result.Result };
